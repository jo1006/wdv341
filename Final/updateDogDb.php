...
try {
    ...
    $sql = "UPDATE Dog 
    SET age = :age, breed = :breed, gender = :gender, owner = :owner, vaccinated = :vaccinated
  ...
  //  $stmt->bindParam(':dogName', $dogName);
	...
	echo "<script> alert('Successfully updated dog'); window.location.href='updateDog.php';</script>;";
	
} catch (PDOException $e){
    echo $e.message;
	echo "<script> alert('An error occured. Please try again later')</script>";
	error_log("Trouble updating dog in db!", 1,"coupon1006@yahoo.com");
}
$conn = null; //close your connection object
header('Location: ./updateDog.php');
?>
